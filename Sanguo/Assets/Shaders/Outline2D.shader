Shader "Custom/Outline2D" {
    Properties 
    {
		...
		_OutLength ("OutLength", Float) = 0.03
		_CutOff ("CutOff", Float) = 0.9
    }
		...
		Pass
		{
			...
			fixed4 frag (v2f i) : COLOR
			{
				...
				if(col.a > _CutOff){
					...
					for (int n=0; n<4; n++)
					{
						fixed4 temp = tex2D(_MainTex, i.texcoord + float2(_OutLength * sin(n * angle + 3.14159 / 4), _OutLength * cos(n * angle + 3.14159 / 4)));
						if (temp.a > sum)
							sum = temp.a;
					}
